...
namespace com.xiaomi.mimc.packet
{
    public class V6PacketDecoder
    {
        ...
        public static V6Packet DecodeV6(byte[] v6HeaderBins, byte[] v6BodyBins, byte[] crcBins, byte[] bodyKey,MIMCUser user)
        {
            ...
            //Console.WriteLine("decodeV6 ============== v6BodyBins.Length:{0}", v6BodyBins.Length);
            ...
            //Console.WriteLine("decodeV6 ============== data v6HeaderBins :{0}", BitConverter.ToString(v6HeaderBins));
            //Console.WriteLine("decodeV6 ============== data v6BodyBins:{0}", BitConverter.ToString(v6BodyBins));
            //Console.WriteLine("decodeV6 ============== data crcBins:{0}", BitConverter.ToString(crcBins));
            //Console.WriteLine("decodeV6 ============== data v6Bins:{0}", BitConverter.ToString(v6Bins));

            uint fecrc = GetUint(crcBins, 0);
            uint crc = Adler32.checkCRC(v6Bins, 0, v6Bins.Length - 4);
            if (fecrc != crc)
            {
                 Console.WriteLine("decodeV6, INVALID_CRC, {0}!={1}", fecrc, crc);
                return null;
            }
            ...
            {
                 Console.WriteLine("decodeV6, INVALID_HEADER, payloadType:{0}, clientHeaderLen:{1}, v6PayloadLen:{2}",
            ...
            // Console.WriteLine("receive v6 packet cmd:{0}, user:{1}", clientHeader.cmd, user.AppAccount());
