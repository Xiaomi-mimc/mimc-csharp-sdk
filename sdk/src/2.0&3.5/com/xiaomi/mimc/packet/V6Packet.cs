        ...
        public byte[] ToByteArray(byte[] v6bodyKey, byte[] payloadKey,MIMCUser user)
        {
            ...
            if (v6BodyBin == null && Body == null)
            {
                ...
                v6PingByteBuffer.putUint(pingcrc);
                 Console.WriteLine("---> send v6 packet Ping appAccount:{0}", user.AppAccount());
            ...
            v6ByteBuffer.putUint(crc);
             Console.WriteLine("---> send v6 packet cmd:{0},appAccount:{1}", Body.ClientHeader.cmd, user.AppAccount());
